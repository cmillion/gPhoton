...
As a test of the relative photometric precision from gPhoton, we plot the difference between the MCAT magnitude and gAperture magnitude against the MCAT magnitude for 10000 and 20000 randomly selected MCAT sources in FUV and NUV respectively. Fig. \ref{fuvrelphot} and Fig. \ref{nuvrelphot} use the background estimated from the gAperture background annulus, while Fig. \ref{fuvrelphotmcat} and Fig. \ref{nuvrelphotmcat} use the per-visit MCAT background. Both methods show agreement within 10\% flux on average, with higher dispersion for dimmer sources. The MCAT background method has a more symmetric distribution overall, especially for dimmer sources, which is consistent with the background result in Section \ref{bgcorr}.
...
\caption{Effective background surface brightness within a $6''$ aperture for the gAperture unmasked annulus method of background methods as compared to values in the visit-level MCAT. Estimates are within $\sim 1\%$ on average in FUV and $\sim 12\%$ on average in NUV. The larger difference in NUV is attributable to more and brighter possible contaminating sources overall.
\label{bgrelphot}}
\end{figure}
...
\caption{Comparison between MCAT and gAperture FUV fluxes for 10000 randomly selected sources, using $6''$ radius aperture annuli and backgrounds estimated from an unmasked annuli extending from $30''$ to $90''$. The dashed line in the left panel denotes the median difference in half magnitude bins. The methods agree within $\sim 4$\% on average with half of the data falling within 14\%.
\label{fuvrelphot}}
\end{figure}
...
\caption{Comparison between MCAT and gAperture NUV fluxes for 20000 randomly selected sources, using $6''$ radius aperture and backgrounds estimated from an unmasked annuli extending from $30''$ to $90''$. The dashed line in the left panel denotes the median difference in half magnitude bins. The methods agree within $\sim 7$\% on average, with a larger offset at dimmer magnitudes, consistent with the contribution of contaminating background stars. Half of the data fall within 18\%.
\label{nuvrelphot}}
\end{figure}
...
\caption{Comparison between MCAT and gAperture FUV fluxes for 10000 randomly selected sources, using $6''$ radius aperture and MCAT background estimates. The dashed line in the left panel denotes the median difference in half magnitude bins. The methods agree within $\sim 5$\% on average. Half of the data fall within 11\%.
...
\caption{Comparison between MCAT and gAperture NUV fluxes for 20000 randomly selected sources, using $6''$ radius aperture and MCAT background estimates. The dashed line in the left panel denotes the median difference in half magnitude bins. The methods agree within $\sim 2$\% on average. Half of the data fall within 8\%.
...
\caption{The left and right panels give the magnitude distributions of MCAT photometry of LDS749B given in Figs. \ref{ldsabsphotfuvmcat} and \ref{ldsabsphotnuvmcat} respectively. The peak density of magnitudes falls within $\sim 1$\% of the reference values in both bands.
\label{magdistmcat}}
\end{figure}
