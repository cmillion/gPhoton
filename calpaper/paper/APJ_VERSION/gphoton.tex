...
\caption{Comparison between MCAT and gAperture NUV (top) and FUV (bottom) photometric measurements, using $6''$ radius aperture and backgrounds estimated from an unmasked annuli extending from $30''$ to $90''$. The dashed line in the left panels denotes the median difference in one magnitude bins. In NUV, photometry using the annulus background method agrees within $\sim 7$\% on average, down to 22.5 AB Magnitude, with a notable increase in offset at magnitudes dimmer than $\sim19$, and half of the data fall within $\sim 15$\%. In FUV, the photometry using the annulus background method agrees within $\sim 2$\%, on average, down to 22.5 AB Magnitude, and half of the data fall within $\sim 8$\%. The divergence for NUV dim sources is consistent with Fig. \ref{bgrelphot} and would be mitigated if background apertures were customized to avoid any nearby, contaminating sources.
...
\begin{figure}[t!]
\includegraphics[scale=0.75]{Fig10a.eps}\\\\
\includegraphics[scale=0.75]{Fig10b.eps}
