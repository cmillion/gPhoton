...
As a test of the relative photometric precision from gPhoton, we plot the difference between the MCAT magnitude and gAperture magnitude against the MCAT magnitude for 1000 and 20000 randomly selected MCAT sources in FUV and NUV respectively. Fig. \ref{fuvrelphot} and Fig. \ref{nuvrelphot} use the background estimated from the gAperture background annulus, while Fig. \ref{fuvrelphotmcat} and Fig. \ref{nuvrelphotmcat} use the per-visit MCAT background. Both methods show agreement within 10\% flux on average, with higher dispersion for dimmer sources. The MCAT background method has a more symmetric distribution overall, especially for dimmer sources, which is consistent with the background result in Section \ref{bgcorr}.
