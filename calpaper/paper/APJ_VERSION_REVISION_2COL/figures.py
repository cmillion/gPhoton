...
def make_kde(data, datarange, bwrange=[0.01, 1]):
    ...
    grid = GridSearchCV(KernelDensity(),
                        {'bandwidth': np.linspace(bwrange[0], bwrange[1], 100)},
                        cv=20, n_jobs=4)
    grid.fit(data[:, None])
...
def data_errors(catmag, band, t, sigma=3., mode='mag', aper=None, bg=None):
    """
    Return the min. and max. fluxes out to a given sigma level.
    """
...
data = {}
for band in bands:
    filename = '{path}/{base}{band}.csv'.format(path=inpath, base=base,
                                                band=band)
    print(filename)
...
for band in bands:
    ...
    for bgmode in dmag.keys():
        ...
        print('Using {n} of {m} {b} sources.'.format(
            n=np.where(ix)[0].size, m=np.array(data[band]['flags']).size,
            b=band))
        ...
        for i, m in enumerate(magrange[:-1]):
            mix = ((np.bitwise_and(np.array(data[band]['flags'].values,
                                            dtype='int16'), 0b11111111) == 0) &
                   (data[band][apertxt] >= m) &
                   (data[band][apertxt] < m+magstep) &
                   np.isfinite(dmag[bgmode]))
        ...
        plt.errorbar(data[band][apertxt][ix], np.array(dmag[bgmode][ix]),
                     yerr=np.array(data[band]['{apertxt}_err'.format(
                         apertxt=apertxt)][ix]), fmt="none", ecolor='b',
                     alpha=0.025)

        plt.plot(data[band][apertxt][ix], dmag[bgmode][ix], '.', color='k',
                 alpha=0.2)

        plt.plot(magrange, -(data_errors(
            magrange, band, 100, sigma=1, aper=aper,
            bg=22 if band == 'NUV' else 23.5)[1]-magrange), 'r--', color='g')
        plt.plot(magrange, (data_errors(
            magrange, band, 100, sigma=1, aper=aper,
            bg=22 if band == 'NUV' else 23.5)[1]-magrange), 'r--', color='g')
        ...
        fig.tight_layout()
        fig.savefig('{path}/Fig0{n}{l}.pdf'.format(
            path=outpath, n='5' if bgmode is 'Annulus' else '4',
            l='a' if band is 'NUV' else 'b'), format='pdf', dpi=1000)
...
for i, band in enumerate(bands):
    ...
    mcat_bg = gt.counts2mag(
        data[band]['mcat_bg.1']*mc.area(gt.aper2deg(4)*60*60), band)
    delta = mcat_bg - gphot_bg
    ix = (np.bitwise_and(np.array(data[band]['flags'].values,
                                  dtype='int16'), 0b11111111) == 0)
    print('Using {n} of {m} {b} sources.'.format(
        n=np.where(ix)[0].size, m=np.array(data[band]['flags']).size, b=band))
    plt.subplot(1, 2, i+1, yticks=[])
    plt.tick_params(labelsize=14)
    plt.hist(delta[ix], bins=bincnt, range=dmagrange, color='k',
...
for i,band in enumerate(['NUV','FUV']):
    ...
    x,y,peak,bandwidth = make_kde(gphot_err[np.where(np.isfinite(gphot_err))],
                                  [0,3.5],bwrange=[0.01,0.1])
    # Report the KDE peak
    print(band, 'gphot', peak)
    plt.hist(gphot_err[np.where(np.isfinite(gphot_err))],bins=50,label='gphot',
                                histtype='step',normed=1)
    plt.plot(x,y)

    mcat_err = np.array(data['NUV']['aper4_err'])[ix]
    x,y,peak,bandwidth = make_kde(mcat_err[np.where(np.isfinite(mcat_err))],
                                  [0,3.5],bwrange=[0.01,0.1])
    # Report the KDE peak
    print(band, 'mcat', peak)
    plt.hist(mcat_err[np.where(np.isfinite(mcat_err))],bins=50,label='mcat',
                               histtype='step',normed=1)
    plt.plot(x,y)
...
for i, band in enumerate(bands):
    ...
    gs.update(left=0.1, right=0.95, wspace=0.02, hspace=0.02, bottom=0.1,
              top=0.95)
    delta_ra = np.array(
        data[band]['ra']-data[band]['racent'])*np.cos(np.array(
            data[band]['dec']))*a
    ...
    ix = np.where((np.bitwise_and(np.array(data[band]['flags'].values,
                  ...
                  (data[band][apertxt] > 0) &
                  (data[band][apertxt] <= maglimits[1]) &
                  np.isfinite(delta_ra) & np.isfinite(delta_dec))
    print('Using {n} of {m} {b} sources.'.format(
        n=np.where(ix)[0].size, m=np.array(data[band]['flags']).size, b=band))
    ...
    plt.plot(x, y)
    plt.axvline(peak, color='k', linestyle='dotted', linewidth=2,
                label='KDE Peak: {p} as'.format(p='{0:.2f}'.format(
                    round(peak, 2))))
    plt.axvline(np.median(delta_ra[ix]), color='r', linestyle='dashed',
                linewidth=4,
                label='Median: {m} as'.format(
                    m=round(np.median(delta_ra[ix]), 2)))
    ...
    plt.axhline(np.median(delta_dec[ix]), color='r', linestyle='dashed',
                linewidth=4,
                label='Median: {m} as'.format(
                    m=round(np.median(delta_dec[ix]), 2)))
    ...
    plt.ylabel('{d} Declination (arcseconds)'.format(d=r'$\Delta$'),
               fontsize=18)
    #fig.tight_layout()
    fig.savefig('{path}/Fig02{l}.pdf'.format(
        path=outpath, l='a' if band is 'NUV' else 'b'), format='pdf', dpi=1000)
...
def magrange(band):
    """
    Return an acceptable range about the reference magnitude, based on band.
    """
    return [refmag[band]-0.06,
            refmag[band]+0.1] if band == 'NUV' else [refmag[band]-0.2,
                                                     refmag[band]+0.6]
...
for band in data.keys():
    ...
    if band is 'FUV':
        ...
        plt.errorbar(
            np.array(data[band]['exptime'])[ix[band]][lix],
            (np.array(data[band]['mag_mcatbgsub'])[ix[band][0][lix]]-
             gt.apcorrect1(radius, band)),
            fmt='x', color='r', alpha=0.3,
            yerr=[np.array(
                data[band]['mag_mcatbgsub_err_1'])[ix[band][0][lix]],
                  np.array(
                      data[band]['mag_mcatbgsub_err_2'])[ix[band][0][lix]]])
    plt.errorbar(
        np.array(data[band]['exptime'])[ix[band]],
        np.array(data[band]['mag_mcatbgsub'])[ix[band]]-gt.apcorrect1(radius,
                                                                      band),
    ...
    plt.plot(t, data_errors(refmag[band], band, t, sigma=nsigma, aper=aper)[0],
             'r--')
    plt.plot(t, data_errors(refmag[band], band, t, sigma=nsigma, aper=aper)[1],
             'r--')
    plt.xlim(tmin, tmax)
    ...
    print('{b}: {n} of {m} ({p}%) within {s}'.format(
        b=band, n=cnt, m=len(ix[band][0]), p=100*cnt/len(ix[band][0]),
        s=nsigma))
    if band is 'FUV':
        ...
        #    b=band,n=cntlix,m=len(ix[band][0][lix]),
        #    p=100*cntlix/len(ix[band][0][lix]),s=nsigma))
...
for i, band in enumerate(['NUV', 'FUV']):
    ...
    plt.axvline(refmag[band], color='g', linestyle='solid', linewidth=4,
                label='Ref: {r} Mag'.format(r=round(refmag[band], 2)))
...
for band in data.keys():
    ...
    plt.errorbar(np.array(data[band]['exptime'])[ix[band]],
                 np.array(data[band][apertxt])[ix[band]]-gt.apcorrect1(
                     gt.aper2deg(aper), band),
                 fmt='.', color='k', alpha=0.1,
                 yerr=[np.array(data[band]['mag_bgsub_err_1'])[ix[band]],
                       np.array(data[band]['mag_bgsub_err_2'])[ix[band]]])
    t = np.arange(tmin+1, tmax+1)
    plt.plot(t, refmag[band]*(t/t), 'k')
    plt.plot(t, data_errors(refmag[band], band, t, sigma=3, aper=aper)[0],
             'r--')
    plt.plot(t, data_errors(refmag[band], band, t, sigma=3, aper=aper)[1],
             'r--')
    plt.xlim(tmin, tmax)
    plt.ylim(magrange(band)[0], magrange(band)[1])
    plt.gca().invert_yaxis()
    ...
    a, b = data_errors(refmag[band],
                       band, np.array(data[band]['exptime'])[ix[band]],
                       sigma=nsigma, aper=aper)
...
for i, band in enumerate(['NUV', 'FUV']):
    ...
    plt.axvline(np.median(mags[ix[band]]), color='r', linestyle='dashed',
                linewidth=4,
                label='Median: {m}'.format(m=round(np.median(
                    mags[ix[band]]), 2)))
    plt.xlim(magrange(band))
    plt.gca().invert_xaxis()
...
for band in ['FUV']:
    ...
    plt.ylabel('{b} gAperture Mag'.format(b=band), fontsize=28)
    ...
    print('w/o legs 1-3... {b}: {n} of {m} ({p}%) within {s} sigma'.format(
        ...
        p=100*cntlix/len(ix[band][0][lix]), s=nsigma))
    plt.text(30, 15.9,
             '{p}% within {s}{sym} (excluding legs 1-3, n={n})'.format(
                 p=100*cntlix/len(ix[band][0][lix]), s=nsigma, sym=r'$\sigma$',
                 n=(len(np.array(
                     data[band]['mag_mcatbgsub'])[ix[band][0][lix]]))),
             fontsize=26)
    plt.tight_layout()
...
for i, band in enumerate(['FUV']):
    ...
    plt.xlabel('{b} gAperture Magnitude'.format(b=band), fontsize=28)
    plt.tick_params(axis='both', which='major', labelsize=22)
    plt.text(magrange(band)[1]-0.05, 6, 'n={n}'.format(
        n=len(mags[ix[band]][lix])), fontsize=26)
    plt.tight_layout()
...
the following for every entry in `FUVStim.csv` and `NUVStim.csv`. It was run on
the server on account of being much, much faster.
    ...
    go
    insert into stimResults select 2010,'150.910025185_75.9819518125','FUV',
    count(*) as theCount, getdate() as theTime from #temp1
    where (((x >= -38279.290277 and x< -35661.0053807) and (y >=34401.4593746
    and y < 37019.7442709))
    or ((x >= 36972.6203787 and x< 39590.905275) and (y >=34397.6778277 and
    y < 37015.962724))
    or ((x >= -38274.199733 and x< -35655.9148368) and (y >=-38410.3354215 and
    y < -35792.0505252))
    or ((x >= 36967.0935025 and x< 39585.3783987) and (y >=-38410.7717538 and
    y < -35792.4868575)))
    go
...
development. There is a lot of redundant code between the FUV and NUV mixture
models, which is also just a holdover from early development.
"""
import triangle
import emcee
...
plt.xlim(0, 17000)
plt.tight_layout()
plt.savefig('{path}/Fig10a.pdf'.format(path=outpath), format='pdf', dpi=1000)
...
ix_bg = np.where(post_prob < .5)
bg_ct = len(ix_bg[0])
plt.errorbar(x[ix_bg], y[ix_bg], yerr=yerr[ix_bg], fmt=",k", marker='.',
             alpha=0.1)
...
plt.tight_layout()
plt.savefig('{path}/Fig10b.pdf'.format(path=outpath), format='pdf', dpi=1000)

# Report some results.
print("MCMC result: "
      "m = {m} (+{m_upper}, -{m_lower}) "
      "b = {b} (+{b_upper} -{b_lower})".format(m=m_mcmc[0], m_upper=m_mcmc[1],
                                               m_lower=m_mcmc[2],
                                               b=b_mcmc[0], b_upper=b_mcmc[1],
                                               b_lower=b_mcmc[2]))
...
for sigma in [3]:
    ...
    for i, band in enumerate(['FUV', 'NUV']):
        ...
        plt.ylabel('{b} {n}{s} Error (AB Mag)'.format(b=band, n=sigma,
                                                      s=r'$\sigma$'),
                   fontsize=24)
        ...
        for mag in magrange:
            ...
            plt.plot(t_raw, mag_err, label='{m}{u}'.format(
                m=mag, u=' Mag' if ((mag == 23) | (mag == 17)) else ''))
        for l in [30]:
            plt.axvline(l, color='k', linestyle='dotted', linewidth=2,
                        label='{n} Seconds'.format(n=l))
        plt.legend(fontsize=22, ncol=2)
    plt.tight_layout()
    plt.savefig('{p}/Fig11.pdf'.format(p=outpath),
...
for i in xrange(len(all_flares)):
    ...
    if i == 0:
        this_figure.text(0.02, 0.5, r'Flux (ergs/cm$^{2}\!$/sec/$\AA$)',
