...
Additionally, the authors and MAST have undertaken to process all of the GALEX data with gPhoton and store the photon level data in a database. As of this writing, the database contains all detected photons up through General Release (GR) 6/7 of the GALEX mission, comprising over 1 trillion events with a total volume over 100Tb. In addition to the standalone calibration pipeline, _gPipeline_, the authors have created tools ("the database tools") for querying and working with output from the photon database. These include
...
Then install the most recent version of gPhoton from the Python Package Index (PyPI) using the command: `pip install -i https://pypi.python.org/pypi gPhoton`
...
**NOTE:** The column definitions for the .csv output from _gAperture_ are in flux. These are the column definitions as of the v1.25.0 build.
