...
#################################################################
## This is the main program in this module.
#################################################################
def gFind(band, skypos, trange):
       	ranges = fGetTimeRanges(band,skypos,maxgap,minexp=options.minexp,trange=trange,verbose=options.verbose,detsize=options.detsize)
...
def check_args(options,args):
	...
	## Make sure the gap parameter is greater or equal to zero, if specified at all.
	print options.gap
	exit(1)
	...
	return (band, skypos, trange, maxgap)
#################################################################
...
if __name__ == '__main__':
	## Create the command-line arguments to parse.
	parser = OptionParser()
	...
	band, skypos, trange, maxgap = check_args(options, args)
	gFind(band, skypos, trange, maxgap)
#################################################################
