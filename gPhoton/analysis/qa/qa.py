...
def checkplot(csvfile,outfile=None,format='png',maxgap=500,imscale=4):
    ...
    n=4 # temporary hacking variable for number of rows in figure
    plt.figure(figsize=(imscale*(len(tranges)),imscale*n))
    for i, trange in enumerate(tranges):
        # Countrate
        plt.subplot(n, len(tranges),i+1+len(tranges)*0)
        plt.xticks([])
        if i==0:
        ...
        # Detector Radius
        plt.subplot(n, len(tranges),i+1+len(tranges)*1)
        plt.xticks([])
        if i==0:
            plt.ylabel('detrad')
        plt.xlim(tlim[0],tlim[1])
